...
Class Vc_Frontend_Editor {
	...
	function getLeftButtons() {
		return array(
			array( 'add_element', '<li class="vc-show-mobile"><a href="#" class="vc_element_button" data-model-id="vc_element" id="vc-add-new-element" title="' . __( 'Add new element', LANGUAGE_ZONE ) . '"></a></li>' ),
			// array('add_row', '<li><a href="#" class="vc_row_button" data-tag="vc_row" id="vc-add-new-row">'.__('Add row', LANGUAGE_ZONE).'</a></li>'),
			array( 'templates', '<li><a href="#" class="vc_templates_button"  id="vc-templates-editor-button" title="' . __( 'Templates', LANGUAGE_ZONE ) . '"></a></li>' )
		);
	}
	...
	function render( $template ) {
		$template = vc_template( 'frontend_editor/' . $template . '.tpl.php' );
		require $template;
	}

	function renderEditButton( $link ) {
		if ( $this->showButton() ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc-load-inline-editor" class="vc-inline-link">' . __( 'Edit with Visual Composer', LANGUAGE_ZONE ) . '</a>';
		}
        return $link;
	}
	...
	function renderRowAction( $actions ) {
		$post = get_post();
		if ( $this->showButton( $post->ID ) ) $actions['edit_vc'] = '<a href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with Visual Composer', LANGUAGE_ZONE ) . '</a>';
		return $actions;
	}
	...
	function loadIFrameJsCss() {
		...
		wp_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_pe.css' ) );
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'jquery-ui-draggable' );
		...
		wp_enqueue_script( 'vc_inline_iframe_js', vc_asset_url( 'js/frontend_editor/vc_page_editable.js' ), array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-draggable' ), '1.0', true );
	}
	...
	function enqueueAdmin() {
		...
		wp_enqueue_script( 'jquery-ui-accordion' );
		wp_enqueue_script( 'jquery-ui-autocomplete' );
		wp_enqueue_script( 'vc_inline_bootstap_js', vc_asset_url( 'lib/bootstrap/js/bootstrap.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		...
		wp_enqueue_script( 'vc_inline_panels_js', vc_asset_url( 'js/frontend_editor/panels.js' ), array( 'vc_inline_models_js' ), WPB_VC_VERSION, true );
		...
		wp_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_fe.css' ) );
	}

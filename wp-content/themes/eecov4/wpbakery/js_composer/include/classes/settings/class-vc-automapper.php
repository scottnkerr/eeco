...
if ( ! class_exists( 'Vc_Automapper' ) ) {
	...
	class Vc_Automapper {
		...
		public function __construct() {
			$this->title = __( 'My shortcodes', LANGUAGE_ZONE );
		}
		...
		public function build() {
			wp_register_script( 'wpb_js_composer_automapper', vc_asset_url( 'js/backend/composer-automapper.js' ), array( 'wpb_js_composer_settings', 'backbone', 'shortcode' ), WPB_VC_VERSION, true ); // TODO: remove to automapper render
		...
		<div class="tab_intro">
			<p
			  class="description"><?php _e( 'Use Visual Composer Shortcode Mapper in order to add your custom or 3rd party vendors shortcodes to the list of Visual Composer elements menu. In order to map shortcode you need it to be already installed on your WordPress site.', LANGUAGE_ZONE ) ?></p>
		</div>
		<div class="vc-automapper-toolbar"><a href="#" class="button button-primary"
											  id="vc-automapper-add-btn"><?php _e( 'Map shortcode', LANGUAGE_ZONE ) ?></a>
		</div>
		...
		<script type="text/html" id="vc-automapper-add-form-tpl">
			<label for="vc-atm-shortcode-string"
				   class="vc-info"><?php _e( 'Please enter shortcode string', LANGUAGE_ZONE ) ?></label>

			<div class="vc-wrapper">
				<div class="vc-string">
					<input id="vc-atm-shortcode-string" placeholder="<?php _e( 'Please enter valid shortcode' ) ?>"
						   type="text" class="vc-atm-string">
				</div>
				<div class="vc-buttons">
					<a href="#" id="vc-atm-parse-string"
					   class="button button-primary vc-parse-btn"><?php _e( 'Parse shortcode', LANGUAGE_ZONE ) ?></a>
					<a href="#" class="button vc-atm-cancel"><?php _e( 'Cancel', LANGUAGE_ZONE ) ?></a>
				</div>
			</div>
			<span
			  class="description"><?php _e( 'Please enter valid shortcode like [my_shortcode first_param="first_param_value"]My shortcode content[/my_shortcode]', LANGUAGE_ZONE ) ?></span>
			</div>
		</script>
		...
		<script type="text/html" id="vc-automapper-form-tpl">
			...
			<div class="vc-wrapper">
				...
				<div class="vc-field vc-tag">
					<label for="vc-atm-tag"><?php _e( 'Tag:', LANGUAGE_ZONE ) ?></label>
					<input type="text" name="tag" id="vc-atm-tag" value="{{ tag }}">
				</div>
				<div class="vc-field vc-description">
					<label for="vc-atm-description"><?php _e( 'Description:', LANGUAGE_ZONE ) ?></label>
					<textarea name="description" id="vc-atm-description">{{ description }}</textarea>
				</div>
				<div class="vc-field vc-category">
					<label for="vc-atm-category"><?php _e( 'Category:', LANGUAGE_ZONE ) ?></label>
				...
				<div class="vc-field vc-is-container">
					<label for="vc-atm-is-container"><input type="checkbox" name="is_container" id="vc-atm-is-container"
															value=""> <?php _e( 'Include content param into shortcode', LANGUAGE_ZONE ) ?>
					</label>
				</div>
			</div>
			<div class="vc-line"></div>
			...
			<div class="vc-buttons">
				<a href="#" id="vc-atm-save"
				   class="button button-primary"><?php _e( 'Save changes', LANGUAGE_ZONE ) ?></a>
				<a href="#" class="button vc-atm-cancel"><?php _e( 'Cancel', LANGUAGE_ZONE ) ?></a>
				<a href="#" class="button vc-atm-delete"><?php _e( 'Delete', LANGUAGE_ZONE ) ?></a>
			</div>
		</script>
		<script type="text/html" id="vc-atm-form-param-tpl">
			...
			<div class="vc-fields">
				<div class="vc-param_name vc-param-field">
					<label><?php _e( 'Param name', LANGUAGE_ZONE ) ?></label>
					<# if(param_name === 'content'){#>
						<span class="vc-content"><?php _e( 'Content', LANGUAGE_ZONE ) ?></span>
						<input type="text" style="visibility: hidden;" name="param_name" value="{{ param_name }}"
							   placeholder="<?php _e( 'Required value', LANGUAGE_ZONE ) ?>" class="vc-param-name"
							   data-system="true">
						<span class="description"
							  style="visibility: hidden;"><?php _e( 'Please use only letters, numbers and underscore.', LANGUAGE_ZONE ) ?></span>
						<# } else { #>
							<input type="text" name="param_name" value="{{ param_name }}"
								   placeholder="<?php _e( 'Required value', LANGUAGE_ZONE ) ?>" class="vc-param-name">
							<span
							  class="description"><?php _e( 'Please use only letters, numbers and underscore.', LANGUAGE_ZONE ) ?></span>
							<# } #>
				</div>
				<div class="vc-heading vc-param-field">
					<label><?php _e( 'Heading', LANGUAGE_ZONE ) ?></label>
					<input type="text" name="heading" value="{{ heading }}"
						   placeholder="<?php _e( 'Required value', LANGUAGE_ZONE ) ?>">
					<span
					  class="description"><?php _e( 'Heading for field in shortcode edit form.', LANGUAGE_ZONE ) ?></span>
				</div>
				<div class="vc-type vc-param-field">
					<label><?php _e( 'Field type', LANGUAGE_ZONE ) ?></label>
					<select name="type">
						<option value=""><?php _e( 'Select field type', LANGUAGE_ZONE ) ?></option>
						<option
						  value="textfield"<?php echo '<# if(type==="textfield") { #> selected="selected"<# } #>' ?>><?php _e( 'Textfield', LANGUAGE_ZONE ) ?></option>
						<option
						  value="dropdown"<?php echo '<# if(type==="dropdown") { #> selected="selected"<# } #>' ?>><?php _e( 'Dropdown', LANGUAGE_ZONE ) ?></option>
						<option
						  value="textarea"<?php echo '<# if(type==="textarea") { #> selected="selected"<# } #>' ?>><?php _e( 'Textarea', LANGUAGE_ZONE ) ?></option>
						<# if(param_name === 'content'){#>
						<option value="textarea_html"<?php echo '<# if(type==="textarea_html") { #> selected="selected"<# } #>' ?>><?php _e('Textarea HTML', LANGUAGE_ZONE) ?></option>
						<# } #>
					</select>
					<span class="description"><?php _e( 'Field type for shortcode edit form.', LANGUAGE_ZONE ) ?></span>
				</div>
				<div class="vc-value vc-param-field">
					<label><?php _e( 'Default value', LANGUAGE_ZONE ) ?></label>
					<input type="text" name="value" value="{{ value }}" class="vc-param-value">
					<span
					  class="description"><?php _e( 'Default value or list of values for dropdown type(separate by comma).', LANGUAGE_ZONE ) ?></span>
				</div>
				<div class="description vc-param-field">
					<label><?php _e( 'Description', LANGUAGE_ZONE ) ?></label>
					<textarea name="description" placeholder="">{{ description }}</textarea>
					<span class="description"><?php _e( 'Explain param.', LANGUAGE_ZONE ) ?></span>
				</div>
			</div>
		</script>
		<?php
		...
		<script type="text/html" id="vc-automapper-item-tpl">
			<label class="vc-automapper-edit-btn">{{ name }}</label>
                <span class="vc-automapper-item-controls">
                    <a href="#" class="vc-automapper-delete-btn" title="<?php _e( 'Delete', LANGUAGE_ZONE ) ?>"></a>
                    <a href="#" class="vc-automapper-edit-btn" title="<?php _e( 'Edit', LANGUAGE_ZONE ) ?>"></a>
                </span>
		</script>
		<?php

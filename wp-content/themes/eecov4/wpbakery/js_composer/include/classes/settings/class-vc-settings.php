...
class Vc_Settings {
	...
	public function setTabs() {
		...
		if ( ! vc_is_network_plugin() || ( vc_is_network_plugin() && is_network_admin() ) ) {
			if ( ! vc_is_updater_disabled() ) $this->tabs['updater'] = __( 'Product License', LANGUAGE_ZONE );
		}
				...
				if ( isset( $params['capabilities']['edit_posts'] ) && $params['capabilities']['edit_posts'] === true ):
					...
					<div id="wpb-settings-group-<?php echo $key ?>" class="accordion-body">
						<div class="visibility settings-block">
							<label
							  for="wpb_composer_access_<?php echo $key ?>"><b><?php _e( 'Visual Composer access', LANGUAGE_ZONE ) ?></b></label>
					...
					<div class="shortcodes settings-block">
						...
						<?php if ( ! isset( $el['content_element'] ) || $el['content_element'] == true ): ?>
							...
							<label>
								<input
								  type="checkbox" <?php if ( isset( $shortcode_settings[$sc_base] ) && (int)$shortcode_settings[$sc_base] == 1 ): ?>checked="true" <?php endif; ?>name="<?php echo self::$field_prefix . 'groups_access_rules[' . $key . '][shortcodes][' . $sc_base . ']' ?>"
								  value="1"/>
						...
						<div class="select-all">
							<a href="#"
							   class="wpb-settings-select-all-shortcodes"><?php echo __( 'Select All', LANGUAGE_ZONE ) ?></a>
							| <a href="#"
								 class="wpb-settings-select-none-shortcodes"><?php echo __( 'Select none', LANGUAGE_ZONE ) ?></a>
						</div>
					</div>
				</div>
			<?php
	...
	public function content_types_field_callback() {
		...
		?>
	<p
	  class="description indicator-hint"><?php _e( "Select for which content types Visual Composer should be available during post creation/editing.", LANGUAGE_ZONE ); ?></p>
	<?php
	}
	...
	<p
	  class="description indicator-hint"><?php _e( 'Check this checkbox to prevent content elements from "stacking" one on top other (on small media screens, eg. mobile).', LANGUAGE_ZONE ); ?></p>
	<?php
	}
	...
	<p
	  class="description indicator-hint"><?php _e( 'Enable the use of custom design options for your site. When checked, custom css file will be used.', LANGUAGE_ZONE ); ?></p>
	<?php
	}
	...
	public function disableIfActivated() {
		if ( ! isset( $this->deactivate_license ) ) $this->deactivate_license = vc_license()->deactivation();
	...
	<div class="wrap vc-settings" id="wpb-js-composer-settings">
		...
		<h2 class="nav-tab-wrapper vc-settings-tabs">
			...
			<a href="#vc-settings-<?php echo $tab ?>"
			   class="vc-settings-tab-control nav-tab<?php echo ( $this->active_tab == $tab ? ' nav-tab-active' : '' ) ?>"><?php echo $title?></a>
			<?php endforeach; ?>
		</h2>
		<?php foreach ( $this->tabs as $tab => $title ): ?>
		<?php if ( $tab == 'element_css' ): ?>
			<form action="options.php" method="post" id="vc-settings-<?php echo $tab ?>"
				  class="vc-settings-tab-content<?php echo ( $this->active_tab == $tab ? ' vc-settings-tab-content-active' : '' ) ?>">
...
EOF;
				...
				<a href="#" class="button vc-restore-button"
				   id="vc-settings-custom-css-reset-data"><?php _e( 'Remove all saved', LANGUAGE_ZONE ) ?></a>
			</form>
			<?php elseif ( $tab == 'automapper' ): ?>
			<form action="options.php" method="post" id="vc-settings-<?php echo $tab ?>"
				  class="vc-settings-tab-content<?php echo ( $this->active_tab == $tab ? ' vc-settings-tab-content-active' : '' ) ?>"<?php echo apply_filters( 'vc_setting-tab-form-' . $tab, '' ) ?>>
				<?php vc_automapper()->renderHtml(); ?>
			</form>
			<?php else: ?>
			<?php $css = $tab == 'color' && $use_custom ? ' color_enabled' : ''; ?>
			<form action="options.php" method="post" id="vc-settings-<?php echo $tab ?>"
				  class="vc-settings-tab-content<?php echo ( $this->active_tab == $tab ? ' vc-settings-tab-content-active' : '' ) . $css ?>"<?php echo apply_filters( 'vc_setting-tab-form-' . $tab, '' ) ?>>
				<?php settings_fields( $this->option_group . '_' . $tab ) ?>
				<?php do_settings_sections( $this->page . '_' . $tab ) ?>
				<?php wp_nonce_field( 'wpb_js_settings_save_action', 'wpb_js_nonce_field' ); ?>
				<?php
				...
				<?php if ( $tab == 'color' ): ?>
				<a href="#" class="button vc-restore-button"
				   id="vc-settings-color-restore-default"><?php _e( 'Restore to defaults', LANGUAGE_ZONE ) ?></a>
				<?php endif; ?>
				<?php if ( $tab === 'updater' ): ?>
				...
				<a href="#" class="button vc-activate-license-button"
				   id="vc-settings-activate-license"><?php empty( $license_activation_key ) ? _e( 'Activate license', LANGUAGE_ZONE ) : _e( 'Deactivate license', LANGUAGE_ZONE ) ?></a>
	...
	protected static function getFileSystem( $url = '' ) {
		if ( empty( $url ) ) $url = wp_nonce_url( 'options-general.php?page=vc_settings', 'wpb_js_settings_save_action' );

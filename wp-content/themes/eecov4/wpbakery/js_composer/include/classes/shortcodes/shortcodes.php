...
---------------------------------------------------------- */
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			$controls_edit = ' <a class="column_edit" href="#" title="' . sprintf( __( 'Edit %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a>';
			$controls_delete = ' <a class="column_clone" href="#" title="' . sprintf( __( 'Clone %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a> <a class="column_delete" href="#" title="' . sprintf( __( 'Delete %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a>';
...
---------------------------------------------------------- */
		...
		protected function outputTitle( $title ) {
			return '<h4 class="wpb_element_title">' . __( $title, LANGUAGE_ZONE ) . '</h4>';
		}
...
if ( ! class_exists( 'WPBakeryShortCode_UniversalAdmin' ) ) {
	abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
		...
		protected function singleParamEditHolder( $param, $param_value ) {
			...
			$output .= ( isset( $param['heading'] ) ) ? '<div class="wpb_element_label">' . __( $param['heading'], LANGUAGE_ZONE ) . '</div>' : ''; //vc_span3

			$output .= '<div class="edit_form_line">'; //vc_span9
			$output .= $this->singleParamEditForm( $param, $param_value );
			$output .= ( isset( $param['description'] ) ) ? '<span class="description clear">' . __( $param['description'], LANGUAGE_ZONE ) . '</span>' : '';
			$output .= '</div>';
...
if ( ! class_exists( 'WPBakeryShortCodesContainer' ) ) {
	abstract class WPBakeryShortCodesContainer extends WPBakeryShortCode {
		...
		public function getColumnControls( $controls, $extended_css = '' ) {
			...
			$controls_edit = ' <a class="column_edit" href="#" title="' . sprintf( __( 'Edit this %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a>';
			$controls_clone = '<a class="column_clone" href="#" title="' . sprintf( __( 'Clone this %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a>';

			$controls_delete = '<a class="column_delete" href="#" title="' . sprintf( __( 'Delete this %s', LANGUAGE_ZONE ), strtolower( $this->settings( 'name' ) ) ) . '"></a>';
		...
		public function contentAdmin( $atts, $content ) {
			...
			if ( isset( $this->settings['params'] ) ) {
				...
				if ( sizeof( $groups ) > 1 ) {
					...
					$key = 0;
					foreach ( $groups as $g ) {
						$output .= '<li><a href="#vc-edit-form-tab-' . $key ++ . '">' . ( $g === '_general' ? __( 'General', LANGUAGE_ZONE ) : $g ) . '</a></li>';
					}
					$output .= '</ul>';
					$key = 0;
					foreach ( $groups as $g ) {

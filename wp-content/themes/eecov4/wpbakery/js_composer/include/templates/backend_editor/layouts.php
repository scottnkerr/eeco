...
class WPBakeryVisualComposerLayoutButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function setup($params) {
        if(empty($params['id']) || empty($params['title']))
            trigger_error( __("Wrong layout params", LANGUAGE_ZONE));
    ...
    public function output($post = null) {
        ...
        $output = '<li class="category-layout wpb-layout-element-button not-column-inherit_o"><a id="'.$this->params['id'].'" data-element="vc_column" data-width="'.$this->params['id'].'" class="'.$this->params['id'].' dropable_el clickable_action" href="#"><span>'.__($this->params['title'], LANGUAGE_ZONE).'</span></a></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerTemplateMenuButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        if(empty($this->params)) return '';
        $output = '<li class="wpb_template_li"><a data-template_id="'.$this->id.'" href="#">'.htmlspecialchars(__($this->params['name'], LANGUAGE_ZONE)).'</a> <span class="wpb_remove_template" title="'.__("Delete template", LANGUAGE_ZONE).'" rel="'.$this->id.'"><i class="icon wpb_template_delete_icon"> </i> </span></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerElementButton implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output($post = null) {
        ...
        $output .= '<li data-element="' . $this->base . '" class="wpb-layout-element-button'.$category_css_classes.$class_out.'"'.$data.'><div class="vc-el-container"><a id="' . $this->base . '" data-tag="'.$this->base.'" class="dropable_el vc-shortcode-link clickable_action'.$class.'" href="#">' . $this->getIcon() . htmlspecialchars(__(stripslashes($this->params["name"]), LANGUAGE_ZONE)) .$description.'</a></div></li>';
        return $output;
    }
}
...
class WPBakeryVisualComposerTemplateMenu implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function output( $only_list = false ) {
        ...
        if($is_empty) $output .= '<li class="wpb_no_templates"><span>'.__('No custom templates yet.', LANGUAGE_ZONE).'</span></li>';
        if($only_list===false) {
            $output .= '</ul></li>';
...
class WPBakeryVisualComposerTemplate_r {
    ...
    protected function getTemplates() {
        ...
        if(!empty($this->templates)) {
			if ( get_option( 'wpb_js_composer_templates_slashes_updated' ) !== 'yes' || vc_get_param( 'vc_strip_template_slashes' ) === 'yes' ) {
...
class WPBakeryVisualComposerNavBar implements WPBakeryVisualComposerTemplateInterface {
    ...
    public function addElementsList() {
		...
		! vc_is_frontend_editor() ?
                        '<li><input id="vc_elements_name_filter" type="text" name="vc_content_filter" placeholder="'.__('Search by element name', LANGUAGE_ZONE).'"/></li>'
                        : ''
                    )
    ...
    public function AddElementsModal($id='') {
        return '<div class="modal wpb-elements-list-modal"'.($id!=='' ? ' id="'.$id.'"' : '').'>
          <div class="modal-header">
                ...
                <!--<h3>'.__('Select Content Element', LANGUAGE_ZONE).'</h3>-->
          </div>
          ...
          <div class="modal-footer hide">
            <button class="btn" data-dismiss="modal" aria-hidden="true">'.__('Close',LANGUAGE_ZONE).'</button>
          </div>
        </div>';
    }
    ...
    public function getElementsModal() {
        ...
        <div class="modal wpb-element-edit-modal">
            ...
            <div class="modal-body wpb-edit-form">
                <div class="vc_row-fluid wpb-edit-form-inner">
                    <img src="images/wpspin_light.gif" alt="spinner" /> '.__('Loading...', LANGUAGE_ZONE).'
                </div>
            </div>
            <div class="modal-footer text-center">
                <button class="button-primary wpb_save_edit_form" data-dismiss="modal" aria-hidden="true">'.__('Save',LANGUAGE_ZONE).'</button>
                <button class="button" data-dismiss="modal" aria-hidden="true">'.__('Cancel',LANGUAGE_ZONE).'</button>
          </div>
        </div>
        </script>
        <script id="wpb-post-custom-css-modal-template" type="text/template">
            <div class="modal wpb-post-custom-css-modal">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h3>'.__('Custom CSS', LANGUAGE_ZONE).'</h3>
                </div>
                <div class="modal-body">
                    <textarea class="wpb_custom_post_css_editor"></textarea>
                    <span class="description">'.__('Enter custom CSS code here. Your custom CSS will be outputted only on this particular page.', LANGUAGE_ZONE).'</span>
                </div>
                <div class="modal-footer text-center">
                    <button class="button-primary wpb_save_edit_form" data-dismiss="modal" aria-hidden="true">'.__('Save',LANGUAGE_ZONE).'</button>
                    <button class="button" data-dismiss="modal" aria-hidden="true">'.__('Cancel',LANGUAGE_ZONE).'</button>
            ...
            <div id="wpb_visual_composer-elements" class="vc_navbar">
                ...
                <div class="vc_navbar-inner clearfix">
                    <!--<div class="container">-->
                        <ul class="vc_nav">
                          <li>
                            <a title="'.__('Visual Composer for WordPress', LANGUAGE_ZONE).'" href="http://vc.wpbakery.com/?utm_campaign=VCplugin_header&utm_source=vc_user&utm_medium=backend_editor" class="vc_logo" target="_blank"></a>
                          </li>
                        </ul>
                        '.$buttons.'
                    <!--</div>-->
    ...
    function buildButtons() {
        ...
        $buttons = array(
            array(
                ...
                '<ul class="vc_nav">
                                <li>
                                    <a class="wpb_add_new_element dropable_el button" id="wpb-add-new-element"><i class="icon"></i>'.__('Add element', LANGUAGE_ZONE).'</a>
                                </li>
                            </ul>'
            ),
            array(
                ...
                '<ul class="vc_nav">
                              <li>
                                <a class="wpb_add_new_row dropable_row button" id="wpb-add-new-row" data-element="vc_row"><i class="icon"></i>'.__('Add row', LANGUAGE_ZONE).'</a>
                                </li>
                            </ul>'
            ),
            array(
                ...
                '<ul class="vc_nav">
                                <li class="vc_dropdown">
                                    <a class="wpb_templates button"><i class="icon"></i>'.__('Templates', LANGUAGE_ZONE).' <b class="caret"></b></a>
            ...
            array(
                ...
                (vc_enabled_frontend() ? '<ul class="vc_nav">
                              <li>
                                <a href="' . vc_frontend_editor()->getInlineUrl() . '" class="wpb_edit_inline dropable_row button" id="wpb-edit-inline"><i class="icon"></i>' . __( 'Frontend Edit', LANGUAGE_ZONE ) . '</a>
                                </li>
                            </ul>' : '')
            ),
            array(
                ...
                '<ul class="vc_nav pull-right wpb-update-button">
                                <li><a class="button" id="wpb-save-post">'.__('Update', LANGUAGE_ZONE).'</a></li>
                            </ul>'
            ),
            array(
                'custom_css',
                '<ul class="vc_nav pull-right">
                        <li><a class="wpb_custom_post_css button" id="wpb-custom-post-css"><i class="icon"></i>'.__('CSS', LANGUAGE_ZONE).'</a></li>
                </ul> '
            )
        );
...
class WPBakeryVisualComposerLayout implements  WPBakeryVisualComposerTemplateInterface {
	...
	public function getContainerHelper() {
		// return '<div class="container-helper">' . __('<h2>No content yet! You should add some...</h2>', LANGUAGE_ZONE) . __("<p>Use the buttons under <a href='javascript:open_elements_dropdown();' class='open-dropdown-content-element'><i class='icon'></i> Content Elements</a> on the top or add <a href='#' class='add-text-block-to-content'><i class='icon'></i> Text block</a> with single click.", LANGUAGE_ZONE) . '</p></div>';
        //return '<div class="container-helper"><span>' . __('<h2>No content yet! You should add some...</h2>', LANGUAGE_ZONE) . __('<p>Click <a href="#" class="add-element-to-layout" title="Add to this column"><i class="icon"></i></a> to add new element inside this column.', LANGUAGE_ZONE) . '</p></span></div>';
        return '';
    }

...
;(function($) { "use strict";
  ...
  Carousel.prototype.showHideControl = function(index) {
    ...
    if(index===0){ this.$left_control.fadeTo(200,0.5)}else{ this.$left_control.fadeTo(200,1)};
    //this.$right_control[index===this.items_count-1 ? 'hide' : 'show']();
    if(index===this.items_count-1){ this.$right_control.fadeTo(200,0.5)}else{ this.$right_control.fadeTo(200,1)};
    if(this.$left_control[index===0]){
    }
  }
  ...
  Carousel.prototype.slide = function (type, next) {
    ...
    if ($.support.transition && this.$element.hasClass('vc-slide')) {
      ...
      if(!this.options.autoHeight) this.recalculateSlidelineHeight($next.find(".vc-inner").height(), true)
    ...
    Carousel.prototype.resizeAction = function() {
      ...
      if (this.options.autoHeight) {
        ...
        this.$items.each(function(){
          var item_height = $(this).find(".vc-inner").height()
          if(item_height > max_height) max_height = item_height
        })
        this.$items.height(max_height)
      } else {
        this.recalculateSlidelineHeight(this.$active.find(".vc-inner").height())
      }

...
class WPBakeryVisualComposerSetup extends WPBakeryVisualComposerAbstract {
    ...
    public function addUpgradeMessageLink() {
        ...
        if(empty($username) || empty($api_key) || empty($purchase_code)) {
            echo ' <a href="'.$this->url.'">'.__('Dowload new version from CodeCanyon.', LANGUAGE_ZONE).'</a>';

        } else {
            echo '<a href="'.wp_nonce_url( admin_url('options-general.php?page=wpb_vc_settings&vc_action=upgrade')).'">'.__('Update Visual Composer now.', LANGUAGE_ZONE).'</a>';
        }
    }
...
class WPBakeryVisualComposerSetupAdmin extends WPBakeryVisualComposerSetup {
    ...
    public function adminNoticeNewVersion() {
        ...
        if(!empty($version))
            echo '<div class="updated"><p>'.sprintf(__('There is a new version of Visual Composer available. <a href="http://bit.ly/vcomposer" target="_blank">Download version <b>%s</b></a>.', LANGUAGE_ZONE), $version).'</p></div>';
    }
...
*/
    ...
    public function registerJavascript() {
        ...
        wp_localize_script( 'wpb_js_composer_js_view', 'i18nLocale', array(
            ...
            'main_button_title_revert' => __( 'Classic editor', LANGUAGE_ZONE ),
            'please_enter_templates_name' => __('Please enter templates name', LANGUAGE_ZONE),
            ...
            'are_you_sure_reset_css_classes' => __('Are you sure taht you want to remove all your data?', LANGUAGE_ZONE),
    ...
    public function jsComposerEditPage() {
        ...
        foreach ($pt_array as $pt) {
            add_meta_box( 'wpb_visual_composer', __('Visual Composer', LANGUAGE_ZONE), Array($this->composer->getLayout(), 'output'), $pt, 'normal', 'high');
        }
    }

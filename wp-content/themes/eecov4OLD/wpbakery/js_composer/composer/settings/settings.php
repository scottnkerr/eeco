...
class WPBakeryVisualComposerSettings {
    ...
    public function __construct($composer) {
        ...
        if(!$this->composer->isNetworkPlugin() || ($this->composer->isNetworkPlugin() && is_network_admin())) {
            if(!$composer->updaterDisabled()) $this->tabs['updater'] = __('Product License', LANGUAGE_ZONE);
        }
...
* @param $callback - function to build section header.
    ...
    public function gutter_callback($args) {
        $value = ($value = get_option(self::$field_prefix.$args['id'])) ? $value : $this->getDefault($args['id']);
        echo '<input type="text" name="'.self::$field_prefix.$args['id'].'" value="'.$value.'" class="css-control"> %';
        echo '<p class="description indicator-hint css-control">'.__('To change default horizontal spacing between columns, enter new value in percents here.', LANGUAGE_ZONE).'</p>';
    }
    ...
    public function responsive_max_callback($args) {
        ...
        echo '<p class="description indicator-hint css-control">'.__('By default content elements "stack" one on top other when screen size is smaller then 480px. Here you can change that value if needed.', LANGUAGE_ZONE).'</p>';
    }
    ...
    public function js_composer_purchase_code_callback() {
        ...
        echo '<p class="description indicator-hint">'. sprintf( __( 'Please enter your CodeCanyon Visual Composer license key, you can find your key by following the instructions on <a href="%s" target="_blank">this page</a>.', LANGUAGE_ZONE ), esc_url( "http://kb.wpbakery.com/index.php?title=Item_Verification_Code" ) ) . '</p>';
    }
    public function envato_api_key_callback() {
        ...
        echo '<p class="description indicator-hint">'.__("You can find API key by visiting your Envato Account page, then clicking the My Settings tab. At the bottom of the page you'll find your account's API key.", LANGUAGE_ZONE).'</p>';
    }
